			...
			<form action="" method="POST" name="idc_button_checkout_form">
				<div class="form-row inline left twothird <?php echo (is_array($level) ? '' : 'total'); ?>">
					<?php if (is_array($level)) { ?>
						<label for="level_select"><?php _e('Select Amount', 'idcommerce'); ?>
				...
				<?php if (is_array($level)) { ?>
					...
					<label for="total"><?php _e('Total', 'ignitiondeck'); ?></label>
						<input type="text" class="total" name="total" id="total" value="" placeholder="<?php echo apply_filters('idc_price_format', $v->level_price); ?>" />
					</div>
				<?php } ?>
				<div class="button-error-placeholder">
					<span class="payment-errors" style="display:none;"><?php _e('Input price is below minimum', 'idcommerce') ?></span>
				</div>
