...
function profile_donations_admin() {
	$settings = add_submenu_page('idf', __('Profile Donations', 'idcommerce'), __('Profile Donations', 'idcommerce'), 'manage_options', 'profile-donations', 'profile_donations_menu');
	...
	if (isset($_GET['creator_profile'])) {
		...
		if ($user_id > 0) {
				...
				if (!empty($settings)) {
					...
					$donate_button = do_shortcode('[idc_button type="button" id="direct-donation-button" product="'.$product_id.'" text="'.__('Donate', 'idcommerce').'" thumb="'.(isset($thumb) ? $thumb : '').'" source=".idc_button_lightbox"]');
