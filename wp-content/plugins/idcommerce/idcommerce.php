...
function memberdeck_styles() {
	...
	if (!empty($settings)) {
		...
		if (is_array($settings)) {
			...
			if (isset($settings['manual_checkout']) && !is_idc_free()) {
				$mc = (string) $settings['manual_checkout'];
			}
			else {
				$mc = '0';
			}
			if (isset($settings['ecb']) && !is_idc_free()) {
				$ecb = $settings['ecb'];
			}
			else {
				$ecb = '0';
...
function memberdeck_disable_others() {
	$get_array = array('payment_settings', 'backer_profile', 'edit-profile', 'creator_profile', 'creator_projects', 'mdid_checkout', 'idc_orders', 'key_valid', 'idc_button_submit');
